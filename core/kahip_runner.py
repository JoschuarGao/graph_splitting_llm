...
def _expected_json_path(params: Dict[str, Any]) -> str:
    """
    按你的 KaHIP.py 写文件的规则推断 JSON 路径：
    ~/Desktop/masterarbeit/result/<safe_place>/<road_type_version>/<YYYY-MM-DD>/
        [lane_weight_version_]edges_partitions_k<k>.json
    """
    home = os.path.expanduser("~")
    base_dir = os.path.join(home, "Desktop", "masterarbeit", "result")
    ...
    if lane_ver:
        fname = f"{lane_ver}_edges_partitions_k{k}.json"
    else:
        fname = f"edges_partitions_k{k}.json"

    return os.path.join(base_dir, safe_place, road_ver, today_str, fname)

def _fallback_find_json(params: Dict[str, Any]) -> Optional[str]:
    """
    兜底：如果预期文件名不在，就在当天目录里找任意 *_edges_partitions_k<k>.json，取最近修改的那个。
    """
    home = os.path.expanduser("~")
    base_dir = os.path.join(home, "Desktop", "masterarbeit", "result")
    safe_place = _safe(params["place"])
